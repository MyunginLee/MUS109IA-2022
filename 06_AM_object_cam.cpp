...
public:
  ...
  float timeflow; // time for overall temporal info / control
  ...
  void onAnimate(double dt) override
  {
    ...
    float tscale = timeflow * 0.001;
    nav().smooth(0.5); // smooth movement
    Vec3f point_you_want_to_see = Vec3f(0,0,-25); // examplary point that you want to see
    nav().faceToward(point_you_want_to_see, Vec3f(0, 1, 0), 0.1);
    nav().moveR(cos(tscale));
    nav().moveU(sin(tscale));
    cout << nav().pos() << endl;}
